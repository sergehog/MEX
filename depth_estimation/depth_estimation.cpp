...
void mexFunction (int nout, mxArray* output[], int in, const mxArray* input[])
{
	...
	if (in < _offset + 4 || (in - _offset - 4) % 2 != 0 || nout != 1 || mxGetClassID(input[3]) != mxSINGLE_CLASS || mxGetClassID(input[4]) != mxSINGLE_CLASS)
	{
		mexPrintf("Depth Estimation for a reference camera\n");
		mexErrMsgTxt("USAGE: [Cost] = depth_estimation(minZ, maxZ, layers, single(ImageRef), single(CRef), Image1, C1, <,..., ImageN, CN>);");
	}
