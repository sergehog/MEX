/** Caculates single iteration of PatchMatch
* @file patchmatch_slanted
* @date 7.05.2016
...
float check_error(const bool direction, const MexImage<const float> &Reference, const MexImage<const float> &Template, const MexImage<float> &Weights, const int x, const int y, const float a, const float b, const float c)
{
	...
	float weights = 0;
	//float pixels = 0;
	#pragma omp parallel for reduction(+:cost_value, weights)
	for (int w = 0; w < window; w++)
	{
		const int dx = w / diameter - radius;
		const int dy = w % diameter - radius;
		...
		float d = a * xx + b * yy + c;
		const float xr = std::min(width-1, std::max(0.0, xx + d*dir));
		float diff = 0;
...
void mexFunction(int nout, mxArray* output[], int in, const mxArray* input[])
{
	...
	{
		mexErrMsgTxt("USAGE: [Al, Ar, ErrL, ErrR] = patchmatch_slanted(single(Left), single(Right), single(LeftGuide), single(RightGuide), radius, maxdisp, [mindisp = 0, iterations=10, forget_rate=0.9, sigma]);");
	}
