...
void mexFunction(int nout, mxArray* output[], int in, const mxArray* input[])
{
	...
	if (in < 7 || nout != 2 || mxGetClassID(input[1]) != mxSINGLE_CLASS || mxGetClassID(input[3]) != mxSINGLE_CLASS)
	{
...
#pragma omp section
		{
			checkValidity(Valid1, C1inv, C2, Z1, Z2, threshold, minZ, maxZ);
		}

#pragma omp section
		{
			checkValidity(Valid2, C2inv, C1, Z2, Z1, threshold, minZ, maxZ);
		}
		}
	}
...
#pragma omp section
		{
			checkValidity(Valid1, C1inv, C2, D1, D2, threshold, minZ, maxZ);
		}

#pragma omp section
		{
			checkValidity(Valid2, C2inv, C1, D2, D1, threshold, minZ, maxZ);
		}
		}
	}
	else
