...
#pragma omp parallel for
	for (long i = 0; i<HW; i++)
	{
		...
		Confidence(x, y) = (second_best - best_value) / float(second_best);
	}
...
#pragma omp parallel for
	for (long i = 0; i<HW; i++)
	{
		...
		if (abs(best - second_best) == 1 && best > 0 && best < layers - 1)
		{
			...
			Disparity(x, y) = float(best) - (cost_next - float(cost_prev)) / (2 * (cost_next + cost_prev - 2 * float(best_value)));
			Disparity(x, y) = abs(Disparity(x, y) - best) >= 1 ? float(best) : Disparity(x, y);
			Disparity(x, y) += mindisp;
			Confidence(x, y) = (third_value - best_value) / float(third_value);
		}
		else
		{
			Disparity(x, y) = mindisp + float(best);
			Confidence(x, y) = (second_value - best_value) / float(second_value);
		}
...
void mexFunction(int nout, mxArray* output[], int in, const mxArray* input[])
{
	...
	const bool interpolate = (in > 2) ? (bool)mxGetScalar(input[2]) : false;
