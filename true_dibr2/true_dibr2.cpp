...
void readMatrix(matrix matrix, const mxArray* array)
{
	const float *data = (float*)mxGetData(array);
	for(int i=0; i<16; i++)
	{
		int k = i / 4;
		int m = i % 4;
...
void projUVZtoXY(matrix projMatrix, float u, float v, float z, float *x, float *y)
{
	double c0, c1, c2;
...
float projXYZtoUV(matrix projMatrix, float x, float y, float z, float *u, float *v, float *zv)
{
	float w;
...
{	
	...
	if(mxIsCell(input[1]))
	{
		...
		for(int i=0; i<cameras; i++)
		{
			...
			//float P_input[4][4];
			readMatrix(P_mats[i], mxGetCell(input[3], i));
	...
	for(size_t camera = 0; camera<cameras; camera++)
	{
		...
		for(long i=0; i<HW; i++)
		{
			...
			float v = height - y - 1;
			float xx = -1, yy = -1;
			projUVZtoXY(P_mats[camera], u, v, zi, &xx, &yy);
			float xv = -1, yv = -1, zv = -1;
			projXYZtoUV(desired, xx, yy, zi, &xv, &yv, &zv);
			yv = height - yv - 1;
