...
void mexFunction(int nout, mxArray* output[], int in, const mxArray* input[])
{
	...
	if (in < 4 || in > 6 || nout != 4 || mxGetClassID(input[0]) != mxSINGLE_CLASS || mxGetClassID(input[1]) != mxSINGLE_CLASS)
	{
		mexPrintf("USAGE: [FilledColor, FilledDepth, Confidence, DataTerm] = inpaint_rgbd_olsson(Color, Depth, UV, OrigDepth, <search, radius>);\n");
	...
	MexImage<float> UV(input[2]);
	MexImage<float> OrigDepth(input[3]);
	...
	if (UV.width != width || UV.height != height || UV.layers != 2)
	{
		mexErrMsgTxt("Wrong UV dimensions!");
	}
	if (OrigDepth.width != width || OrigDepth.height != height || OrigDepth.layers != 2)
	{
		mexErrMsgTxt("Wrong OrigDepth dimensions!");
	}


	const int _search = (in > 4) ? std::abs((int)mxGetScalar(input[4])) : 0;
	const int search = _search ? _search : std::max(width, height);
	const int radius = (in > 5) ? std::max(1, (int)mxGetScalar(input[5])) : 3;
