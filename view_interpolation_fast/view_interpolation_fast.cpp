...
void mexFunction (int nout, mxArray* output[], int in, const mxArray* input[])
{
	...
	if (in < _offset + 4 || nout != 3 || mxGetClassID(input[0]) != mxSINGLE_CLASS)
	{
		...
		mexErrMsgTxt("USAGE: [Desired, Depth, BestCost] = view_interpolation_fast(C_desired, minZ, maxZ, layers, sigma, single(Image1), C1, single(Image2), C2, <,..., ImageN, CN>);");
	}
